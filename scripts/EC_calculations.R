...
#'helper function to get length_delta csv
#'
...
get_data <- function(drug, file){
  ...
  if (drug %in% drugs_drc3){
    ...
    write_file <- paste0("projects/Olivia/cbriggsae_drc/data/",drc3,drug, "/delta_slopes.csv")
  }else{
    pathway_drc <- paste0(directory,drc4,drug, file_name)
    write_file <- paste0("projects/Olivia/cbriggsae_drc/data/",drc4,drug, "/delta_slopes.csv")
  }
...
# going ahead with LL.5 model since all drugs have the lowest IC with that value # 
# and LL.4 model was have issue with the EC50 values #
...
#' new function with new LL.5 model
#' 
...
#'
#'@param fit output from safe_LL5_strain (model fitting from drc::drm function utilizing LL.5() model)
...
#code for the ec10 heritability 
temp_merged <- rownames_to_column(EMO_EC, var = "rowid")
ECA2666 <- temp_merged %>% 
...
final_EC_table <- paste0("projects/Olivia/cbriggsae_drc/data/combined_data/total_EC_estimations_LL.4.csv")
...
final_slope_table <- paste0("projects/Olivia/cbriggsae_drc/data/combined_data/total_slope_calculations_LL.4.csv")
...
#anova comparison no strain and strain model there are more anova tests see line 2542 (slope.aov)
#do later
...
get_EC_comp <- function(drug, drug_name){
  ...
  EDcomps.df <- suppressMessages(data.frame(drc::EDcomp(fit$result, num_conc)))
...
final_slopecomp_table <- paste0("projects/Olivia/cbriggsae_drc/data/combined_data/total_slope_comparisons.csv")
...
total_ec_comps <- total_ec_comps %>% 
  dplyr::select(drug, comps, Estimate, Std..Error, t.value, p.value, sig_p_val)
final_eccomp_table <- paste0("projects/Olivia/cbriggsae_drc/data/combined_data/total_ec_comparisons.csv")
write.csv(total_ec_comps, file = final_eccomp_table, row.names = FALSE)

n.slope.comp.tests <- slope.comps %>%
  ...
  dplyr::mutate(N2.normed.estimate = if_else(strain1 == "AF16", true = 1/Estimate, false = Estimate))
...
#' @export
compare_EC_to_heritable_dose <- function(heritability_data, ec_data, drug_name) {
...
#' @return Data frame with H² at doses closest to EC10, EC50, EC90
#' @export
find_heritability_at_EC <- function(drug, ec_data, drug_name) {
  drug_data <- get_data(drug,"20260617")
...
find_heritability_at_EC("ABZ", temp_merged, "Albendazole")
find_heritability_at_EC("FBZ", temp_merged, "Fenbendazole")
find_heritability_at_EC("IVM", temp_merged, "Ivermectin")
find_heritability_at_EC("MOX", temp_merged, "Moxidectin")
find_heritability_at_EC("MILB", temp_merged, "Milbemycin")
find_heritability_at_EC("LEV", temp_merged, "Levamisole")
find_heritability_at_EC("PYR", temp_merged, "Pyrantel")
find_heritability_at_EC("CLO", temp_merged, "Closantel")
find_heritability_at_EC("EMO", temp_merged, "Emodepside")
