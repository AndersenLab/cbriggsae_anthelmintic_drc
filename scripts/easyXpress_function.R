#' DRC_briggs
#'
#' this function will loop through the imputed DRC numbers and drugs from the NemaSize data
#' to run the following custom easyXpress functions adapted for data from NemaSize:
#'  checkBalance(),checkEff(), checkWF(), filterWF(), titerWF(), nWF(), outlierWF(), delta(), regEff()
#'
#'@param drc_number folder name for specific DRC(s) you want to look at (vector input)
#'@param data_pathway pathway to all the DRC folder 
#'@param output_pathway pathway to directory you want figures and data folder to be
#'@param drugs drug abbreviation for the drug(s) you want to look at (vector input) 
#'@param date date you ran script (organization purposes)
#'@param design_drc boolean for if you want to combine DRC2 and 3 only works for the following drugs: 
#'"ABZ", "CLO", "EMO", "FBZ", "IVM", "LEV", "PYR"

DRC_briggs <- function(drc_number = c("20250424_Cbriggsae_DRC2","20250602_Cbriggsae_DRC3", "20250619_Cbriggsae_DRC4"),
                       data_pathway = "projects/Amanda/projects/nemasize/projects/",
                       output_pathway = "projects/Olivia/cbriggsae_drc/",
                       drugs = c("ABZ", "CLO", "DEC", "EMO", "FBZ", "IVM", "LEV", "MOX", "PYR", "MILB"),
                       date = "20260615",
                       combine_drc = TRUE){
  ...
  # implementing a loop to go through all drugs 
  ###########################################
  #                                         #
  #                 NEMASIZE                #
  #                                         #
  ...
  # drugs in dose response 4 (moxidectin (MOX) and milbemycin (MILB) mix)
  
  final_data_folder <- paste0(output_pathway,"data/")
  final_figure_folder <- paste0(output_pathway, "figures/")
  
  ifelse(!dir.exists(file.path(final_data_folder)),
         dir.create(file.path(final_data_folder)),
         "Directory Exists")
  
  ifelse(!dir.exists(file.path(final_figure_folder)),
         dir.create(file.path(final_figure_folder)),
         "Directory Exists")
  ...
  for (a in drc_number){
    ...
    for (b in drugs){
      ...
      all_values <- list(titer_Cb_ns = titer_Cb_ns, well_summary_ns = well_summary_ns)
      
      if (!combine_drc){
        curr_figure_dir <- paste0(output_pathway,"figures/", a,"_",b)
      }else{
        curr_figure_dir <- paste0(output_pathway,"figures/", "DRC2andDRC3_",b)
      }
      
      ifelse(!dir.exists(file.path(curr_figure_dir)),
             dir.create(file.path(curr_figure_dir)),
             "Directory Exists")
      
      #function to ask if you want to save a plot or not 
      save_plot <- function(func_call = "easyXpress", variable = x, save_path = curr_figure_dir, fig_width = 6, fig_height = 4){
        the_user_prompt <- paste0("Do you want to save the plot from ", func_call, "()? \n 1-YES 2-NO: ")
        will_save <- "" 
        while (will_save == ""){
          will_save <- readline(prompt = the_user_prompt)
          if (will_save != ""){
            if (will_save == 1){
              ggsave(filename= paste0(func_call,"_Cb_plot.png"), device = png, plot = variable, 
                     path = save_path, dpi = 300, width = fig_width, height = fig_height)
              cat(paste0("file saved as ",func_call, "_Cb_plot.png \n"))
            } else if (will_save == 2){
              cat(paste0("The ", func_call ," plot will not be saved \n"))
            } else{
      ...
      save_plot(func_call = "titerWF", variable = titer_Cb_plot_ns)
      ...
      save_plot(func_call = "nWF", variable = n_Cb_plot_ns)
      n_Cb_data_ns <- n_Cb_ns$d
      
      #save value to list 
      all_values <- c(all_values, list(n_Cb_ns = n_Cb_ns))
      ...
      message("-- merged the nWF results with the summarized wells results -> merged_summarize_Cb_ns")
      ...
      all_values <- c(all_values, list(merged_summarize_Cb_ns = merged_summarize_Cb_ns))
      # outlierWF
      ...
      #save plot
      save_plot(func_call = "checkWF", variable = cw_Cb_ns$p , fig_width = 8, fig_height = 12)
      
      # Look at the summary data
      cw_Cb_ns$d
      
      #save the variable 
      all_values <- c(all_values, list(cw_Cb_ns = cw_Cb_ns))
      ...
      all_values <- c(all_values, list(outlier_sum = outlier_summary))
      ...
      save_plot(func_call = "summarize_outlier_types", variable = outlier_graph)
      ...
      #save value to return
      all_values <- c(all_values, list(fw_Cb_ns_length = fw_Cb_ns_length))
      ...
      #save value to return
      all_values <- c(all_values, list(fw_Cb_ns = fw_Cb_ns))
      ...
      nrow(fw_Cb_ns_length)  # after filtering
      ...
      #save the plot 
      save_plot(func_call = "checkBalance", variable = cb_Cb_plot_ns, fig_height = 7)
      ...
      #save value to return
      all_values <- c(all_values, list(cb_Cb_ns = cb_Cb_ns))
      ...
      #function to return drug specific axis marks 
      ...
      #function to return drug specific labels 
     ...
     fw_Cb_ns_length %>%
      ...
      #save the plot 
      save_plot(func_call = "checkEff", variable = checkeff_Cb_ns, fig_width = 17, fig_height = 8)
      ...
      if(a == "20250424_Cbriggsae_DRC2" || combine_drc || a == "20250619_Cbriggsae_DRC4"){
        ...
        save_plot(func_call = "regEff(length)_p2", variable = reg_Cb_ns_by_conc$p2)
        save_plot(func_call = "regEff(length)_p1", variable = reg_Cb_ns_by_conc$p1, fig_height = 6)
        
        #save value to return
        all_values <- c(all_values, list(reg_Cb_ns_by_conc = reg_Cb_ns_by_conc))
        ...
        save_plot(func_call = "regEff(width)_p2", variable = reg_Cb_ns_by_conc_width$p2)
        save_plot(func_call = "regEff(width)_p1", variable = reg_Cb_ns_by_conc_width$p1, fig_height = 6)
        
        #save value to return
        all_values <- c(all_values, list(reg_Cb_ns_by_conc_width = reg_Cb_ns_by_conc_width)) 
        ...
        save_plot(func_call = "delta(length)", variable = delta_length, fig_width = 17, fig_height= 8)
        
        #save value to return
        all_values <- c(all_values, list(del_length_Cb_ns = del_length_Cb_ns))
        ...
        #save value to return
        all_values <- c(all_values, list(del_width_Cb_ns = del_width_Cb_ns))
        
        save_plot(func_call = "delta(width)", variable = delta_width, fig_width = 17, fig_height = 8)
        
        
        #make a new directory for each DRC and drug to store the data tables
        if(!combine_drc){
            new_dir <- paste0("projects/Olivia/cbriggsae_drc/data/",a,"_", b)
        }else{
          new_dir <- paste0("projects/Olivia/cbriggsae_drc/data/","DRC2andDRC3_",b)
        }
        ...
        write_file1 <- paste0(new_dir,"/", date,"_length_reg_delta_nemasize.csv")
        write_file2 <- paste0(new_dir,"/", date,"_width_reg_delta_nemasize.csv")
      ...
      write_file1 <- paste0(new_dir,"/", date,"_length_delta_nemasize.csv")
      write_file2 <- paste0(new_dir,"/", date,"_width_delta_nemasize.csv")
     }
      
      # these will be your final cleaned df per drug
      write.csv(del_length_Cb_ns,
                file = write_file1,
                row.names = FALSE)
      
      write.csv(del_width_Cb_ns,
                file = write_file2,
                row.names = FALSE)
      ...
      if (a == "20250424_Cbriggsae_DRC2" || a == "20250619_Cbriggsae_DRC4"){
        ...
        #save value to return
        all_values <- c(all_values, list(all_Cb_response_NS = all_Cb_response_NS))
      ...
      dose_average_NS$strain <- factor(dose_average_NS$strain)
      
      write_file <- paste0(new_dir,"/mean_median_length_delta_nemasize.csv")
      write.csv(dose_average_NS,file = write_file, row.names = FALSE)
      
       #save value to return
      all_values <- c(all_values, list(dose_average_NS = dose_average_NS))
      ...
      #save value to return
      all_values <- c(all_values, list(SE.DRC_NS = SE.DRC_NS))
      
      save_plot(func_call = "final_DRC_graph", variable = SE.DRC_NS, fig_width = 12, fig_height = 6)
